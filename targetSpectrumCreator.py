...
def targetSpectrum(lat , lon , vs30, intensity):
    ...
    T_list  = [ item  for item in np.linspace(0.0, 5.0, 1001)]
...
# function to create TBEC-2018 target spectrum vertical
def verticalTargetSpectrum(lat , lon , vs30, intensity):
    """
    Args:
        lat (float): latitude of the location
        lon (float): longitude of the location
    ...
    for i in T_list:
        ...
        if i <TAD:
            Sad.append(round((0.32 + 0.48*(i/TAD))*SDs, 4))
            
        elif i >= TAD and i<=TBD:
            Sad.append(round(0.8*SDs, 4))
            
        elif i>TBD and i <=TLD:
            Sad.append(round((0.8*SDs*TBD/i), 4))

    target_spec = {"T" : T_list[: len(Sad)],
                   "Sad" : Sad}

    target_spec_vertical_df = pd.DataFrame().from_dict(target_spec)
    
    return target_spec_vertical_df
